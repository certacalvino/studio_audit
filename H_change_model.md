...
> 2. **Proposal detail tab** — full spec built and validated. Sticky header with Promote button, lifecycle stepper, CRs per env, Scope + Activity two-column, AI panel in Review state.
...
4. **Open Proposals** — a single compact link row (`✦ Open Proposals · 5 · View all in Overview →`), not a full list. The full list lives in Overview; the Studio app only signals it exists.
...
Early drafts explored a shared "Navigator" block (search + 4 category cards) rendered inside *both* Overview and the Studio app, reasoning that they overlap in audience. Decided against it — **PM confirmed Overview stays exactly as it is** (stats, pipeline, full Open Proposals list, Recent Activity feed). No merge needed. The two pages serve different questions and don't need shared UI to feel connected:
...
They're linked by cross-reference (the Studio app's Open Proposals link points at Overview), not by shared components.
...
- Center: global search `🔍 Search apps, workflows, proposals… ⌘K`
...
1. **Stats row** — 4 cards, 3 attention (Open Proposals across apps · Assigned to you for review · Blocked) + 1 context (Apps count · sandbox/test hidden).
...
3. **Starred apps** — 3-column tile grid. Each tile: name + star + 4 env dots + workflow/record counts + open proposals + last edited.
4. **Recently active** — same tile format, ordered chronologically.
5. **All apps** — table (Name / Environments / Open Proposals / Drafts / Last Edited). Sort dropdown + `Hide sandbox/test` toggle ON by default (from audit's original spec — dedupes the 453-row noise).
...
- **Studio app → Open Proposals** section → click a row
...
- Studio AI is scoped to the app and **proactive on Discovery** (Studio app + Overview Insights), **contextual on Execution** (workflow tabs), **review-oriented on Review** (Proposal tabs), and **carrying memory across sessions**.
...
|---|---|---|
| **Naming** — container entity | ✓ Resolved (v2) | **Proposal** |
...
| **Proposal tab AI state name** | ✓ Resolved (v3) | **Review** — chips: Summarize diff · Suggest reviewers · Check compliance risks |
...
- **App Overview** with 3 sub-tabs (Summary / Insights / Activity) — Summary has enriched Pipeline (per-env sub-metadata + active deploy indicator), Needs your attention, Team row, Open Proposals with Live badge + reviewer designation, Recent Activity with deploy events
- **Proposal detail tab** — sticky header with Promote → QA button, lifecycle stepper (6 nodes), Change Requests per env (with real draft IDs — `CR #461 → DEV-5430`, `CR #483 → QA-1207`), Scope + Activity two-column
- **Studio app "+ New Tab"** — plain search bar (not AI-branded) · Recent · Object categories with Create/Import icons · Open Proposals compact link · gray bg + card treatment matching Overview
- **AI state-aware content** for Discovery (Overview / Dashboard-less / Studio app) · Ready (Workflow tab without task) · Execution (Workflow with active task) · Review (Proposal tab)
...
- ✓ Studio app page built (with search + Recent + object categories + Open Proposals link)
- ✓ Proposal detail tab spec drafted
- ✓ Naming resolved (Proposal)
- ✓ App identity solved (Mayo Client App as leftmost tab, not chrome duplicate)
- ✓ Notification panel + avatar menu built

**Pending:**
1. **Sync v3 back to Notion** (`Studio: Nav Model & AI Placement`) — currently blocked on Notion MCP; will complete when reconnected.
2. **Verify Proposal detail tab** rendered in Claude Design (last prompt sent — awaiting screenshot).
3. **Relabel existing Change Requests page as "Proposals list"** — mostly relabel, not a rebuild.
4. **Empty state for Open Proposals** (Overview + Studio app + Dashboard).
